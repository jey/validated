    ...
    _createClass(Context, [{
      ...
      value: function buildMapping(validateValue) {
        ...
        if (state.ch !== '{') {
          ...
          throw this.error('Expected a mapping but got ' + (0, _utils.typeOf)(val));
        } else {
          var _object = object(state, function (key, valueState, keyState) {
            valueState = white(valueState);
            var valueContext = new Context(valueState, 'While validating value at key "' + key + '"', _this2);
            var keyContext = new Context(keyState, 'While validating key "' + key + '"', _this2);

            var _validateValue = validateValue(valueContext, key, keyContext),
                context = _validateValue.context,
                value = _validateValue.value;

            return { state: context.state, value: value };
          }),
              _val = _object.value,
      ...
      value: function buildSequence(validateValue) {
        ...
        if (state.ch !== '[') {
          ...
          throw this.error('Expected an array but got ' + (0, _utils.typeOf)(val));
        } else {
          var _array = array(state, function (idx, valueState) {
            valueState = white(valueState);
            var valueContext = new Context(valueState, 'While validating value at index ' + idx, _this3);
